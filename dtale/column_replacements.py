...
class ColumnReplacement(object):
    ...
    def __init__(self, data_id, col, replacement_type, cfg):
        self.data_id = data_id
        if replacement_type == 'spaces':
            self.builder = SpaceReplacement(col, cfg)
        elif replacement_type == 'strings':
            self.builder = StringReplacement(col, cfg)
        elif replacement_type == 'value':
            self.builder = ValueReplacement(col, cfg)
        elif replacement_type == 'imputer':  # iterative, knn, simple
            self.builder = ImputerReplacement(col, cfg)
        else:
...
class SpaceReplacement(object):
    ...
    def build_code(self, data):
        ...
        return "df.loc[:, '{col}'] = df['{col}'].replace(r'^\\s+$', {value}, regex=True)".format(
            col=self.col, value=value
        )
...
class StringReplacement(object):
    ...
    def build_code(self, data):
        ...
        return (
            ...
            "df.loc[:, '{col}'] = df['{col}'].replace(regex_pat, {replace}, regex=True)"
        ).format(col=self.col, regex_exp=regex_exp, flags=flags, replace=replace_with)
...
class ValueReplacement(object):
    ...
    def build_code(self, data):
        ...
        code.append("df.loc[:, '{col}'] = s".format(col=self.col))
        return '\n'.join(code)
...
class ImputerReplacement(object):
    ...
    def build_column(self, data):
        ...
        if imputer_type == 'iterative':  # Sklearn 0.21+ is needed
            imputer = IterativeImputer()
        elif imputer_type == 'knn':  # Sklearn 0.22+ is needed
            n_neighbors = self.cfg.get('n_neighbors') or 2
            imputer = KNNImputer(n_neighbors=n_neighbors)
        elif imputer_type == 'simple':  # Sklearn 0.21+ is needed
    ...
    def build_code(self, _data):
        ...
        code.append(
            "df.loc[:, '{col}'] = pd.DataFrame(output, columns=['{col}'], index=df.index)['{col}']".format(col=self.col)
        )
        return '\n'.join(code)
